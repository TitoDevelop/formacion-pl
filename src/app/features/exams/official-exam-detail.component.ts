    ...
    @else if (exam(); as e) {
      ...
      <div class="launch-layout">
        <section class="panel official-launch-main">
          <h2>¿Cómo quieres realizarlo?</h2><p class="muted">El contenido es el mismo; cambia la forma de corrección.</p>
          <div class="launch-mode-grid">
            <button class="launch-mode" [class.selected]="mode==='EXAM'" (click)="mode='EXAM'">
              <span class="launch-mode-icon">📝</span><div><strong>Modo examen</strong><p>No verás la corrección hasta terminar.</p></div><span class="radio-dot"></span>
            </button>
            <button class="launch-mode" [class.selected]="mode==='PRACTICE'" (click)="mode='PRACTICE'">
              <span class="launch-mode-icon">⚡</span><div><strong>Modo práctico</strong><p>Corrige cada respuesta al instante.</p></div><span class="radio-dot"></span>
            </button>
          </div>
          <button class="btn primary wide launch-button" (click)="start()">INICIAR {{ mode==='EXAM' ? 'EXAMEN' : 'PRÁCTICA' }}</button>
        </section>
...
export class OfficialExamDetailComponent implements OnInit {
  exam = signal<OfficialExam | null>(null); loading = signal(true); mode: TestMode = 'EXAM';
  constructor(private route: ActivatedRoute, private router: Router, private data: DataService) {}
  async ngOnInit() { try { this.exam.set(await this.data.getExam(this.route.snapshot.paramMap.get('id')!)); } finally { this.loading.set(false); } }
  start() { this.router.navigate(['/app/oficiales', this.exam()!.id, 'realizar'], { queryParams: { mode: this.mode } }); }
}
