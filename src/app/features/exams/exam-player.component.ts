...
@else if (exam()) {
  <div class="exam-topline"><div><span class="eyebrow">{{ mode()==='PRACTICE' ? 'OFICIAL · MODO PRÁCTICO' : 'EXAMEN OFICIAL' }}</span><h1>{{ exam()!.municipality }} · {{ exam()!.year }}</h1></div><div class="exam-progress-actions"><div class="test-timer" aria-label="Tiempo transcurrido">⏱ {{ formatTime(timer.elapsedSeconds()) }}</div><div class="progress-text">{{ currentIndex()+1 }} / {{ questions().length }}</div></div></div>
...
export class ExamPlayerComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(){ const id=this.route.snapshot.paramMap.get('id')!; this.mode.set(this.route.snapshot.queryParamMap.get('mode')==='PRACTICE'?'PRACTICE':'EXAM'); try{ const [exam,questions]=await Promise.all([this.data.getExam(id),this.data.getExamQuestions(id)]); this.exam.set(exam); this.questions.set(questions); this.marked.set(await this.data.reviewQuestionIds(questions.map(q=>q.question_id))); if(questions.length)this.timer.start(this.router.url); }finally{this.loading.set(false);} }
  ...
  async finish(){ if(this.mode()==='EXAM'&&!confirm('¿Finalizar el examen y ver la corrección?'))return; this.submitting.set(true); try{ const payload=this.questions().map(row=>{const id=this.selected()[row.question_id]??null;const c=row.questions.question_options?.find(o=>o.is_correct);return{questionId:row.question_id,selectedOptionId:id,correct:!!id&&id===c?.id}}); const e=this.exam()!; const timing=this.timer.snapshot(); const attemptId=await this.data.finishAttempt(e.id,'OFFICIAL',this.mode(),e.name,null,payload,timing); this.timer.clear(); await this.router.navigate(['/app/resultado',attemptId]); }finally{this.submitting.set(false);} }
}
