  ...
  template: `
    @if (loading()) {
      ...
      <div class="exam-topline">
        <div><span class="eyebrow">EXAMEN OFICIAL</span><h1>{{ exam()!.municipality }} · {{ exam()!.year }}</h1></div>
      ...
      @if (current(); as row) {
        <article class="question-card">
          <div class="question-meta">Pregunta {{ row.question_number || currentIndex()+1 }}</div>
...
export class ExamPlayerComponent implements OnInit {
  ...
  progress = computed(() => this.questions().length ? ((this.currentIndex()+1) / this.questions().length) * 100 : 0);
  ...
  async ngOnInit() {
    ...
    try {
      const [exam, questions] = await Promise.all([this.data.getExam(id), this.data.getExamQuestions(id)]);
      this.exam.set(exam);
  ...
  letter(pos: number) { return ['A','B','C','D'][pos-1] ?? '?'; }
  select(questionId: string, optionId: string) { this.selected.update(s => ({...s, [questionId]: optionId})); }
  ...
  async finish() {
    ...
    try {
      const payload = this.questions().map(row => {
        ...
        return { questionId: row.question_id, selectedOptionId: optionId, correct: !!optionId && optionId === correctOption?.id };
      });
      const attemptId = await this.data.finishAttempt(this.exam()!.id, 'OFFICIAL', payload);
